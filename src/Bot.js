...
export class Bot extends Player {
  ...
  constructor(modes = Bot.Modes, sign = -1) {
    super();
    this.modes = modes;
    this.sign = sign;
  }
  ...
  reset(columns, rows) {
    ...
    this.cycleMode =
      this.modes[(this.modes.indexOf(this.mode) + 1) % this.modes];
    this.cycleCount = 0;
    this.recent = [];
  }

  fixPath(cells) {
    const coords = cells.flatMap((inner, x) => inner.map((cell, y) => [x, y]));
    this.path = new Set(
      Array.from(coords.filter(([x, y]) => this.isOwned(cells[x][y])))
    );
  }
  ...
  performAction(cells, opponent) {
    ...
    const multiplier = getNeighbours(tx, ty).filter(
      ([a, b]) => cells[a] && this.isOwned(cells[a][b])
    ).length;
    const basePower = this.counter.value;
    this.counter.value = 0;
    const power = this.sign * basePower * multiplier;

    const result = cells[tx][ty] + power;
    if (this.isOwned(result)) {
      this.path.add(target);
    }


    console.debug("## mode", this.sign, mode, tx, ty, power);

    return [tx, ty, power];
  }

  isOwned(value) {
    return value !== 0 && Math.sign(value) === this.sign;
  }
  ...
  expand(cells) {
    ...
    const outer = squares
      ...
      .filter(([sx, sy]) => cells[sx] && !this.isOwned(cells[sx][sy]));
